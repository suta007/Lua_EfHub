...
local function GetFruitConfig1()
	return {
		...
		WeightValue = tonumber(Options.ipWeightValue.Value) or 100
	}
end

local function GetFruitConfig2()
	return {
		...
		WeightValue = tonumber(Options.ipWeightValue2.Value) or 100
	}
end

CheckFruit = function(model, config)
	...
	if config.CheckVariant then
		...
		local isVariantMatch = (tVariant == config.VariantType)
...
CheckFruit1 = function(Fruit)
	return CheckFruit(Fruit, GetFruitConfig1())
end

CheckFruit2 = function(Fruit)
	return CheckFruit(Fruit, GetFruitConfig2())
end
...
local function CollectFruitWorker(mode)
	local isEnabled, queue, isScanning, scanMode
	
	if mode == 1 then
		isEnabled = Options.tgCollectFruitEnable.Value
		queue = FruitQueue1
		isScanning = IsScanning1
		scanMode = 1
	elseif mode == 2 then
		isEnabled = Options.tgCollectFruitEnable2.Value
		queue = FruitQueue2
	...
	if #queue > 0 then
		...
		-- Because IsScanning is globally defined as IsScanning1 and IsScanning2,
		-- we pass scanMode to ScanFarmTask depending on whether we scan for Farm 1 or 2
		if not isScanning then ScanFarmTask(scanMode) end
		task.wait(0.5)
		return
	end
...
CollectFruitWorker1 = function() CollectFruitWorker(1) end
CollectFruitWorker2 = function() CollectFruitWorker(2) end
